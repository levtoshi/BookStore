...
{
    public class AddBookStockCommand : AsyncCommandBase, IDisposable
    {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            try
            {
                await _bookService.AddBookStockAsync(_selectedProduct.ProductId, _changeBookStockViewModel.Amount);
                await _productsStore.RefreshAsync();
                _navigationService.Navigate();
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error adding book to stock: {ex.Message}", "Error", MessageBoxButton.OK);
            }
        }
